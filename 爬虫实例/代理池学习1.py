...
def spider(pages, max_change_porxies_times=300):
    """
    抓取 XiciDaili.com 的 http类型-代理ip-和端口号西刺代理
    ...
    for i in range(pages):
        ...
        while True:
            ...
            if content.status_code == 503:  # 如果503则ip被封,就更换ip
                proxies = get_proxies()
                try_times += 1
                print('第{{0}):0>3s}次变更,当前{1}'.format(str(try_times),proxies))
        ...
        print('正在抓取第{0}页数据,共{1}页'.format(i+1,pages))
        for j in range(2, 102):  # 用简单的xpath提取http,host和port
            tree = etree.HTML(content.text)
            http = tree.xpath('//table[@id="ip_list"]/tr[{0}]/td[6]/text()'.format(j))[0]
            host = tree.xpath('//table[@id="ip_list"]/tr[{0}]/td[2]/text()'.format(j))[0]
            port = tree.xpath('//table[@id="ip_list"]/tr[{0}]/td[3]/text()'.format(j))[0]
